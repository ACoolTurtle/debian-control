...
const mut = require("../lib/control");

const BZFLAG_CONTROL_OBJ = {
...
const BZFLAG_SERVER_CONTROL = `Package: bzflag-server
Source: bzflag (2.4.18-1)
Version: 2.4.18-1+b1
Installed-Size: 3967
Maintainer: Debian Games Team <pkg-games-devel@lists.alioth.debian.org>
...
const BZIP2_DSC_NO_SIGNATURE = `Format: 3.0 (quilt)
Source: bzip2
Binary: libbz2-1.0, libbz2-dev, bzip2, bzip2-doc
Architecture: any all
Version: 1.0.6-8.1
...
const readFileAsync = filename => new Promise((resolve, reject) => fs.readFile(filename, {encoding: "utf8"}, function(err, data) {
    if (err != null) { return reject(err); }
    return resolve(data);
}));

describe("control", function() {
    it("should supply some functions", function() {
        mut.should.have.properties(CONTROL_FUNCTIONS);
        return Array.from(CONTROL_FUNCTIONS).map((func) =>
            mut[func].should.be.a.Function());
    });

    describe("isContinueLine", () => it("should recognize continuation lines", function() {
        ...
        return mut.isContinueLine("\tbdbe7bf29e014e44d79bb7c733fe63cae990ab50882a4a07867cf69c61ad72b7 59875 bzip2_1.0.6-8.1.debian.tar.bz2").should.equal(true);
    }));

    describe("isFieldLine", () => it("should recognize field lines", function() {
        ...
        return mut.isFieldLine("\tbdbe7bf29e014e44d79bb7c733fe63cae990ab50882a4a07867cf69c61ad72b7 59875 bzip2_1.0.6-8.1.debian.tar.bz2").should.equal(false);
    }));

    describe("parse", function() {
        it("should be able to parse a control file without a signature", function() {
            const obj = mut.parse(BZIP2_DSC_NO_SIGNATURE);
            return obj.should.eql(BZIP2_CONTROL_OBJ);
        });

        it("should be able to parse a control file with a signature", function() {
            const bzip2dsc = await(readFileAsync("src/test/data/bzip2_1.0.6-8.1.dsc"));
            const obj = mut.parse(bzip2dsc);
            return obj.should.eql(BZIP2_CONTROL_OBJ);
        });

        return it("should be able to parse a control file including a multiline", function() {
            const obj = mut.parse(BZFLAG_SERVER_CONTROL);
            return obj.should.eql(BZFLAG_CONTROL_OBJ);
        });
    });

    describe("stringify", () => it("should be able to stringify a control object", function() {
        const obj = mut.parse(BZIP2_DSC_NO_SIGNATURE);
        obj.should.eql(BZIP2_CONTROL_OBJ);
        const control = mut.stringify(BZIP2_CONTROL_OBJ);
        return control.should.eql(BZIP2_DSC_NO_SIGNATURE);
    }));

    return describe("stripSignature", function() {
        it("should strip the signature of a PGP signed control file", function() {
            const bzip2dsc = await(readFileAsync("src/test/data/bzip2_1.0.6-8.1.dsc"));
            const stripBzip2dsc = mut.stripSignature(bzip2dsc);
            return stripBzip2dsc.should.equal(BZIP2_DSC_NO_SIGNATURE);
        });

        return it("should pass a control file without a PGP signature", function() {
            const bzip2dsc = await(readFileAsync("src/test/data/bzip2_1.0.6-9.1.not-signed.dsc"));
            const stripBzip2dsc = mut.stripSignature(bzip2dsc);
            stripBzip2dsc.should.equal(bzip2dsc);
            return (stripBzip2dsc === bzip2dsc).should.equal(true);
        });
    });
});
