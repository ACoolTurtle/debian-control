...
const mut = require("../lib/lineStream");

const BZIP2_DSC = `-----BEGIN PGP SIGNED MESSAGE-----
Hash: SHA512
...
describe("lineStream", function() {
    it("should supply a LineStream class", function() {
        mut.LineStream.should.be.ok();
        return mut.LineStream.should.be.a.Function();
    });

    return it("should break things down line by line", function(done) {
        const RESULT = (Array.from(BZIP2_DSC.split("\n")).map((x) => `${x}\n`)).reverse();
        const dscFile = fs.createReadStream("src/test/data/bzip2_1.0.6-8.1.dsc");
        const ls = new mut.LineStream();
        ls.on("data", line => line.toString().should.equal(RESULT.pop()));
        ls.on("end", function() {
